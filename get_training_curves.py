...
for epoch in range(nbr_epoch):
    train_file = open(f"training_list_dice/train/train_logs_list_{epoch}.pkl", "rb")
    train_data = pickle.load(train_file)
    for f in train_data:
        f["epoch"] = epoch
    train_L_dice += train_data

    valid_file = open(f"training_list_dice/valid/valid_logs_list_{epoch}.pkl", "rb")
...
for epoch in range(nbr_epoch):
    train_file = open(f"training_list_cross/train/train_logs_list_{epoch}.pkl", "rb")
    train_data = pickle.load(train_file)
    for f in train_data:
        f["epoch"] = epoch
    train_L_cross += train_data

    valid_file = open(f"training_list_cross/valid/valid_logs_list_{epoch}.pkl", "rb")
    valid_data = pickle.load(valid_file)
    for f in valid_data:
        f["epoch"] = epoch
...
#Here we take care of the false iou score values
#We also take the mean of the iou functions on one epoch
...
#We get the results for the dice and crossentropy loss
...
plt.savefig('training_curves/training_curves.png')
plt.show()
